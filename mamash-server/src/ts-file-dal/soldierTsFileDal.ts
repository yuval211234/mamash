...
import { logger } from "../loggers/file-logger";
...
let soldiers = soldierWithoutDefaults.map((soldier): Soldier => ({
...soldier,
status: 'undefined',
reason: ''
}));
...
const changeSoldierStatus = (soldier, newStatus) => {
    ...
    logger.info({...soldier, prevStatus, action: 'statusChange'});
}

export const soldierTsFileDal: SoldierDal = {
    getAllSoldiers: () => {
        return soldiers;
    },
    getSoldiersByTeam: (id: string) => {
        const soldiersInTeam = soldiers.filter(soldier => soldier.teamId === id);
        return soldiersInTeam;
    },
    changeSoldierStatusById: (id: string, newStatus: SoldierStatus, reason: string) => {
    ...
    resetAllSoldiers: () => {
        ...
        logger.info({action: 'resetStatusForAllTeams'});

        return true;
    },
    resetAllSoldiersByTeam: (id: string) => {
        soldiers = soldiers.map(soldier => {
        ...
        logger.info({teamId: id, action: 'resetStatusForTeam'});
